...
# get_length_of_headline()
# def get_length_of_headline(headline):
#   return len(headline.split())

def get_length_of_headline(headline):
  """
  Return the number of words in a string headline
...
def find_average_word_length(headlines, total_words):
...
def find_total_words(headlines):
  total_words = 0
  for headline in headlines:
      # Split each headline into words and count them
      total_words += get_length_of_headline(headline)
  return total_words


# Calculate and display the average (mean)
total_words = find_total_words(headlines)
average_words = find_average_word_length(headlines, total_words)
